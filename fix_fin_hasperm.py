...
context_pattern = r"const { penaltyRequests, teams, setPenaltyRequests, notify, establishments } = useContext\(AppContext\);"
...
if "const hasPerm =" not in content:
    content = content.replace(context_pattern, replacement)

with open("src/components/FinancialReports.jsx", "w", encoding="utf-8") as f:
    f.write(content)

print("Fixed hasPerm in FinancialReports")
