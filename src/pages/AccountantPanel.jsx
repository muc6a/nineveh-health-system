...
import { LogOut, DollarSign, Activity, FileText, CheckCircle2, ShieldAlert, BadgeInfo, BellRing, Sun, Moon, Cloud, ChevronLeft, CreditCard, Banknote, Search, AlertCircle, Eye, ClipboardList, Menu, LayoutDashboard, Printer, Mail, Inbox, Archive, Filter, Building, Compass, Map, CheckSquare, X } from 'lucide-react';
                      ...
                      {dailyInventories
                        ...
                        <tr key={inv.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30">
                          <td className="p-3 text-slate-700 dark:text-slate-300" dir="ltr" style={{textAlign: 'right'}}>
                            {(() => { const d = new Date(inv.date); return isNaN(d) ? inv.date : `${d.getDate()}-${d.getMonth() + 1}-${d.getFullYear()} | ${d.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }).toLowerCase()}`; })()}
                          </td>
