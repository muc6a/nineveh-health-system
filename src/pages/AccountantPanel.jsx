...
export const AccountantPanel = () => {
  ...
  const getEstablishmentSector = (estId) => {
    const est = establishments.find(
      (e) => String(e.id).toLowerCase() === String(estId).toLowerCase(),
    );
  ...
  const allFines = (penaltyRequests || []).filter(
    (r) => r.type === "fine" || r.type === "closure",
  );
  ...
  const sectorFines = allFines.filter((f) => {
    ...
    return (
      targetSector === "الكل" ||
      estSector === targetSector ||
      (estSector && estSector.includes(targetSector))
    );
  });
  ...
  const sectorMonthlyRevenue = sectorMonthlyPaid.reduce(
    (sum, f) => sum + (f.amount || 0),
    0,
  );
  const sectorTotalRevenue = paidFines.reduce(
    (sum, f) => sum + (f.amount || 0),
    0,
  );
  const sectorPendingAmount = pendingFines.reduce(
    (sum, f) => sum + (f.amount || 0),
    0,
  );
  ...
  const todayPaidFines = paidFines.filter(
    (f) => new Date(f.paymentDate).toDateString() === today,
  );
  const cashCollected = todayPaidFines
    .filter((f) => f.paymentMethod === "cash")
    .reduce((sum, f) => sum + (f.amount || 0), 0);
  const posCollected = todayPaidFines
    .filter((f) => f.paymentMethod === "pos")
    .reduce((sum, f) => sum + (f.amount || 0), 0);
  ...
  const markDirectiveRead = (id) => {
    setDirectives((prev) =>
      prev.map((d) => (d.id === id ? { ...d, isRead: true } : d)),
    );
  };
  ...
  const compMonthlyPaid = comprehensiveFilteredFines.filter(
    (f) =>
      f.paymentStatus === "paid" &&
      new Date(f.paymentDate).getMonth() === currentMonth &&
      new Date(f.paymentDate).getFullYear() === currentYear,
  );
  const compMonthlyRevenue = compMonthlyPaid.reduce(
    (sum, f) => sum + (f.amount || 0),
    0,
  );
  const compTotalRevenue = comprehensiveFilteredFines
    ...
    setTimeout(() => {
      try {
        ...
        const fuzzyMatch = (a, b) =>
          safeString(a).replace(/[-_\s]/g, "") ===
          safeString(b).replace(/[-_\s]/g, "");

        // Broaden the search: try exact ID, fuzzy ID, QR extracted ID, or partial name
        let est = (establishments || []).find(
          (e) =>
        ...
        let fineByFineId = (allFines || []).find(
          (f) => safeString(f.id) === code || fuzzyMatch(f.id, code),
        );
        ...
        if (fineByFineId) {
          ...
          const targetEstId = safeString(
            targetFine.establishmentId || targetFine.estId,
          );
          targetEst = (establishments || []).find(
            (e) =>
              safeString(e.id) === targetEstId || fuzzyMatch(e.id, targetEstId),
          );
        } else if (est) {
          targetEst = est;
          const estId = safeString(est.id);
          // Find the fine in allFines, prioritize pending fines if multiple exist
          const estFines = (allFines || []).filter(
            (f) =>
              safeString(f.establishmentId || f.estId) === estId ||
              fuzzyMatch(f.establishmentId || f.estId, estId),
          );
          targetFine =
            estFines.find((f) => f.paymentStatus !== "paid") ||
            estFines[0] ||
            null;
        }
        ...
        if (targetEst) {
          ...
          if (targetFine) {
            ...
            if (targetFine.paymentStatus === "paid") {
              notify(
                "تنبيه: تم العثور على غرامة سابقة ولكنها مسددة.",
                "warning",
              );
            }
          } else {
            ...
            notify(
              "تم العثور على المنشأة، ولا توجد غرامات معلقة عليها.",
              "info",
            );
          }
        } else {
          ...
          setSearchedFine(null);
          notify(
            "المنشأة غير متوفرة في قاعدة البيانات ولم يتم العثور على غرامة مسجلة بهذا الكود",
            "error",
          );
        }
      } catch (error) {
  ...
  const handleCloseRegister = (status) => {
    if (status === "confirm") {
      ...
      notify(
        "تم تأكيد صحة الجرد وإغلاق الصندوق وتوثيق العملية الدائمة بنجاح",
        "success",
      );
    } else {
            ...
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl overflow-hidden shadow-sm">
              <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                <h4 className="text-sm font-bold text-slate-800 dark:text-white">
                  أحدث الغرامات المعلقة للقطاع
                </h4>
                <span className="px-2 py-1 bg-red-100 text-red-600 dark:bg-red-500/20 dark:text-red-400 rounded text-[10px] font-black">
                  {pendingFines.length} غرامة
                </span>
              </div>
              {pendingFines.length === 0 ? (
                <div className="p-8 text-center text-slate-500 text-xs font-bold">
                  لا توجد غرامات معلقة حالياً.
                </div>
              ) : (
                        ...
                        <tr key={fine.id}>
                          <td className="p-3 text-slate-700 dark:text-slate-300">
                            {getEstablishmentName(
                              fine.establishmentId || fine.estId,
                            )}
                          </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 dark:bg-slate-950/80 backdrop-blur-sm">
            <div className="w-full max-w-4xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-2xl relative text-right max-h-[90vh] overflow-y-auto">
              <button
                ...
                className="absolute top-6 left-6 p-2 bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 rounded-full text-slate-500 transition-colors"
              >
                <X className="w-5 h-5" />
              </button>
              ...
              <div className="space-y-6 animate-fade-in-up">
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-sm max-w-xl mx-auto">
                  <p className="text-xs text-slate-500 font-bold mb-4 text-center">
                    قم بإدخال كود المنشأة (أو رقم الغرامة) لاسترجاع التفاصيل
                    وتسديد المبلغ فوراً.
                  </p>
                  <div className="flex gap-2">
                    <input
                      ...
                      value={searchCode}
                      onChange={(e) =>
                        setSearchCode(e.target.value.toUpperCase().trim())
                      }
                      onKeyDown={(e) => {
                        if (e.key === "Enter") handleSearchFine();
                      }}
                  ...
                  <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 p-6 rounded-3xl shadow-lg w-full mx-auto relative overflow-hidden">
                    ...
                    <div className="mb-6 border-b border-slate-100 dark:border-slate-800 pb-4">
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        <div>
                          <span className="block text-[10px] text-slate-500 font-bold mb-1">
                            اسم المنشأة
                          </span>
                          <span className="block text-sm font-black text-slate-800 dark:text-white">
                            {searchedEstablishment.name}
                          </span>
                        </div>
                        <div>
                          <span className="block text-[10px] text-slate-500 font-bold mb-1">
                      ...
                      <>
                        ...
                        <div className="grid grid-cols-2 gap-4 mb-6 relative z-10">
                          <div className="bg-slate-50 dark:bg-slate-800/50 p-4 rounded-xl border border-slate-100 dark:border-slate-700">
                            ...
                            <span className="block text-3xl md:text-4xl font-black text-red-600 py-1 drop-shadow-sm">
                              {searchedFine.amount?.toLocaleString()}{" "}
                        ...
                        <>
                          <div>
                            ...
                            <div className="grid grid-cols-2 gap-3">
                              ...
                              >
                                <CreditCard className="w-5 h-5" /> إلكتروني
                                (POS)
                              </button>
                            </div>
                          </div>
                          ...
                          <div className="flex gap-3 pt-4">
                            ...
                            <button
                              onClick={() => {
                                notify("جاري التجهيز لطباعة الوصل...", "info");
                ...
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 shadow-sm">
                  ...
                  {paidFines.length === 0 ? (
                    <div className="p-8 text-center text-slate-500 text-xs font-bold bg-slate-50 dark:bg-slate-800/30 rounded-2xl">
                      لا توجد وصولات مسددة في الأرشيف.
                    </div>
                  ) : (
                    <div className="overflow-x-auto">
                      <table className="w-full text-right border-collapse text-xs font-bold">
                          ...
                          {paidFines
                              ...
                              (f) =>
                                ...
                                (f.receiptNumber &&
                                  f.receiptNumber
                                    .toLowerCase()
                                    .includes(
                                      archiveSearchTerm.toLowerCase(),
                                    )) ||
                                getEstablishmentName(
                                  f.establishmentId || f.estId,
                                )
                                  .toLowerCase()
                            ...
                            .map((fine) => (
                              ...
                              >
                                ...
                                <td className="p-3">
                                  <span
                                    className={`px-2 py-1 text-[10px] rounded-lg ${fine.paymentMethod === "pos" ? "bg-indigo-100 text-indigo-700 dark:bg-indigo-500/20 dark:text-indigo-400" : "bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400"}`}
                                  >
                                    {fine.paymentMethod === "pos"
                                      ? "POS"
                                      : "نقدي"}
                                  </span>
                                </td>
                                  ...
                                  <div className="flex flex-col gap-1">
                                    ...
                                    <span className="text-[9px] bg-slate-100 dark:bg-slate-800 text-slate-500 px-1.5 py-0.5 rounded font-bold w-fit">
                                      ...
                                      {Math.ceil(
                                        ...
                                        ? "بنفس اليوم"
                                        : `${Math.ceil(Math.abs(new Date(fine.paymentDate) - new Date(fine.timestamp || fine.date || fine.paymentDate)) / (1000 * 60 * 60 * 24))} يوم`}
                                    </span>
                                  </div>
                                </td>
                              </tr>
        ...
        {activeTab === "reconciliation" && (
          <div className="space-y-6 animate-fade-in-up">
            <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-6 md:p-8 shadow-sm relative">
              ...
              <p className="text-xs md:text-sm text-slate-500 font-bold mb-8">
                ملخص الإيرادات المالية لهذا اليوم لمطابقتها مع الصندوق الفعلي
                قبل الإغلاق.
              </p>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-8">
                <div className="bg-emerald-50 dark:bg-emerald-900/20 border border-emerald-100 dark:border-emerald-800/50 p-6 rounded-2xl text-center">
                  ...
                  <span className="block text-3xl font-black text-emerald-700 dark:text-emerald-400">
                ...
                <div className="bg-indigo-50 dark:bg-indigo-900/20 border border-indigo-100 dark:border-indigo-800/50 p-6 rounded-2xl text-center">
                  ...
                  <span className="block text-3xl font-black text-indigo-700 dark:text-indigo-400">
                    {posCollected.toLocaleString()}
                  </span>
                  <span className="block text-xs text-indigo-600/70 mt-2">
                    دينار عراقي
                  </span>
                </div>
              </div>
              ...
              <div className="bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 p-6 rounded-2xl flex items-center justify-between mb-8">
                <span className="text-base font-bold text-slate-600 dark:text-slate-300">
                  إجمالي عدد وصولات القبض المصدرة اليوم:
                </span>
                <span className="text-2xl font-black text-slate-800 dark:text-white bg-white dark:bg-slate-700 px-4 py-2 rounded-xl border border-slate-200 dark:border-slate-600 shadow-sm">
                  {totalReceipts} وصولات
                </span>
              </div>
                      ...
                      {dailyInventories
                        .filter(
                          (inv) =>
                            ...
                            inv.date.startsWith(inventoryArchiveDate),
                        )
                        .map((inv) => (
                          <tr
                            key={inv.id}
                          ...
                          >
                            ...
                            >
                              {(() => {
                                ...
                                return isNaN(d)
                                  ? inv.date
                                  : `${d.getDate()}-${d.getMonth() + 1}-${d.getFullYear()} | ${d.toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: true }).toLowerCase()}`;
                              })()}
                            </td>
                            <td className="p-3 text-emerald-600 font-black">
            ...
            <div className="space-y-6 animate-fade-in-up">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="bg-amber-50 dark:bg-amber-500/10 border border-amber-200 dark:border-amber-500/20 p-6 rounded-3xl relative overflow-hidden">
                  ...
                  <span className="block text-sm font-bold text-amber-700 dark:text-amber-400 mb-2">
                    الإيرادات المستحصلة (هذا الشهر) - {reportFilter}
                  </span>
                  <span className="block text-4xl font-black text-amber-800 dark:text-amber-300 relative z-10">
                    {compMonthlyRevenue.toLocaleString()}{" "}
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs font-bold text-amber-600 dark:text-amber-500/80">
                    ...
                    <span className="bg-amber-200 dark:bg-amber-500/30 px-2 py-1 rounded-lg">
                      {compMonthlyPaid.length}
                    </span>
                  </div>
                </div>

                <div className="bg-slate-100 dark:bg-slate-800/50 border border-slate-200 dark:border-slate-700/50 p-6 rounded-3xl">
                  <span className="block text-sm font-bold text-slate-600 dark:text-slate-400 mb-2">
                    إجمالي الإيرادات التراكمية (منذ الإطلاق) - {reportFilter}
                  </span>
                  <span className="block text-4xl font-black text-slate-800 dark:text-white">
                    {compTotalRevenue.toLocaleString()}{" "}
                  ...
                  <div className="mt-4 flex items-center justify-between text-xs font-bold text-slate-500 dark:text-slate-400">
                    ...
                    <span className="bg-slate-200 dark:bg-slate-700 px-2 py-1 rounded-lg">
                      {
                        comprehensiveFilteredFines.filter(
              ...
              <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-sm">
                <div className="p-5 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center">
                  <h4 className="text-sm font-black text-slate-800 dark:text-white">
                    تفاصيل الإيرادات الأخيرة ضمن هذا النطاق
                  </h4>
                </div>
                      ...
                      {comprehensiveFilteredFines
                        ...
                        .map((fine) => (
                          ...
                          >
                            ...
                            <td className="p-4 text-teal-600 dark:text-teal-400">
                              {getEstablishmentSector(
                                fine.establishmentId || fine.estId,
                              )}
                            </td>
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white dark:bg-slate-900 rounded-2xl w-full max-w-4xl max-h-[85vh] flex flex-col shadow-2xl overflow-hidden animate-fade-in-up">
            <div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50 shrink-0">
              <h3 className="font-black text-slate-800 dark:text-white flex items-center gap-2">
                {selectedReportType === "monthly" ? (
                  <>
                    <Banknote className="w-5 h-5 text-emerald-500" /> تفاصيل
                    الإيرادات المحصلة (هذا الشهر)
                  </>
                ) : selectedReportType === "total" ? (
                  <>
                    <Activity className="w-5 h-5 text-teal-500" /> تفاصيل إجمالي
                    الإيرادات (الكلية)
                  </>
                ) : (
                  <>
                    <AlertCircle className="w-5 h-5 text-red-500" /> تفاصيل
                    الغرامات المعلقة (قيد التسديد)
                  </>
                )}
              </h3>
              <button
                ...
                className="p-2 bg-slate-200 dark:bg-slate-700 rounded-xl hover:bg-red-100 hover:text-red-600 dark:hover:bg-red-500/20 dark:hover:text-red-400 transition-colors"
              >
                <X className="w-4 h-4" />
              </button>
            </div>
            <div className="flex-1 overflow-y-auto p-0">
              <table className="w-full text-right border-collapse text-xs font-bold">
                <thead className="bg-slate-50 dark:bg-slate-800/80 sticky top-0 z-10 shadow-sm">
                  <tr>
                    <th className="p-4 border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">
                      المنشأة
                    </th>
                    <th className="p-4 border-b border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400">
                      رقم الوصل
                ...
                <tbody className="divide-y divide-slate-100 dark:divide-slate-800">
                  {(() => {
                    ...
                    if (dataList.length === 0) {
                      return (
                        <tr>
                          ...
                          >
                            لا توجد سجلات لعرضها في هذا القسم.
                          </td>
                        </tr>
                      );
                    }
                    ...
                    return dataList.map((item) => {
                      ...
                      const targetEst = establishments.find(
                        (e) =>
                          e.id === item.targetEstId ||
                          e.id === item.estId ||
                          e.id === item.establishmentId,
                      );
                      ...
                      return (
                        ...
                        >
                          <td className="p-4 text-slate-800 dark:text-slate-200">
                            {item.estName || targetEst?.name || "غير معروف"}
                          </td>
                          <td
                          ...
                          <td className="p-4 text-slate-800 dark:text-slate-200 font-black">
                          ...
                          >
                            {(() => {
                              const d = new Date(
                                isPaid ? item.paymentDate : item.date,
                              );
                              return isNaN(d)
                                ? isPaid
                                  ? item.paymentDate
                          ...
                          <td className="p-4">
                            {isPaid ? (
                              <span className="px-2 py-1 bg-emerald-100 text-emerald-700 dark:bg-emerald-500/20 dark:text-emerald-400 rounded-md text-[10px]">
                                مسددة
                              </span>
                            ) : (
                              <span className="px-2 py-1 bg-red-100 text-red-700 dark:bg-red-500/20 dark:text-red-400 rounded-md text-[10px]">
                                غير مسددة
                              </span>
                            )}
                          </td>
            ...
            <div className="p-4 bg-slate-50 dark:bg-slate-800 border-t border-slate-200 dark:border-slate-700 flex justify-end shrink-0">
              <button
                onClick={() => setSelectedReportType(null)}
                className="px-6 py-2 bg-slate-200 hover:bg-slate-300 dark:bg-slate-700 dark:hover:bg-slate-600 text-slate-800 dark:text-white rounded-xl font-bold text-xs transition-colors"
              >
                إغلاق
              </button>
            </div>
