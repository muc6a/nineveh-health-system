...
export const ExecutivePortal = ({ embeddedTab }) => {
  ...
  React.useEffect(() => {
    ...
    if (activeTab === 'strategic' && !(hasPerm('showMainDashboard') || hasPerm('showReportsPage') || hasPerm('exportData'))) needsRedirect = true;
    if (activeTab === 'operations_room' && !(hasPerm('authenticatePenalties'))) needsRedirect = true;
    ...
    <>
      <div className={`bg-slate-50 dark:bg-slate-950 flex transition-colors duration-300 relative ${embeddedTab ? 'min-h-full rounded-2xl overflow-hidden' : 'min-h-screen'}`}>
        ...
        <UnifiedSidebar 
          ...
          executiveTab={executiveTab} setExecutiveTab={setExecutiveTab}
          isSidebarOpen={isSidebarOpen} setIsSidebarOpen={setIsSidebarOpen}
          allowedTeams={allowedTeams} selectedTeamId={selectedTeamId} setSelectedTeamId={setSelectedTeamId}
        />
        )}
