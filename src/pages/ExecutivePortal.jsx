        ...
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 dark:bg-white/5 backdrop-blur-md">
          <div className="w-full max-w-3xl bg-slate-900 dark:bg-white border border-slate-700 dark:border-slate-200 p-6 rounded-3xl text-white dark:text-slate-800 shadow-2xl relative max-h-[85vh] overflow-y-auto text-right">
            <div className="flex items-center justify-between pb-4 border-b border-slate-800 dark:border-slate-100 mb-4 sticky top-0 bg-slate-900 dark:bg-white z-10">
              ...
              <button onClick={() => setShowStatsModal(false)} className="p-1.5 rounded-xl hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-500 border border-slate-700 dark:border-slate-200">
                <X className="w-5 h-5" />
              </button>
            </div>
                ...
                if (selectedSector) {
                  ...
                  return (
                    <div className="space-y-3">
                      <h4 className="font-bold text-slate-300 dark:text-slate-700 mb-2">قطاع: {selectedSector}</h4>
                    ...
                    {Object.entries(groupedBySector).map(([sector, count]) => (
                      ...
                      >
                        ...
                        <div className="w-12 h-12 rounded-full bg-teal-500/10 flex items-center justify-center mx-auto mb-2">
                          <span className="text-xl font-extrabold text-teal-400 dark:text-teal-600">{count}</span>
                        </div>
      ...
      {showUninspectedModal && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 bg-slate-950/80 dark:bg-white/5 backdrop-blur-md transition-all">
          <div className="w-full max-w-md bg-slate-900/95 dark:bg-white/95 backdrop-blur-2xl p-6 border border-red-500/20 dark:border-red-500/20 shadow-2xl shadow-red-500/10 rounded-3xl relative overflow-hidden">
            ...
            <p className="text-xs text-slate-300 dark:text-slate-700 mb-4 leading-relaxed text-right font-bold">
              أحياء وشوارع متبقّية بانتظار الزيارة والتفتيش هذا الشهر:
            </p>

            <div className="p-4 rounded-2xl bg-red-950/30 dark:bg-red-50 border border-red-500/20 text-xs font-black text-red-400 dark:text-red-700 max-h-40 overflow-y-auto leading-relaxed text-right shadow-inner">
            ...
            <div className="space-y-3 max-h-60 overflow-y-auto pr-1">
              {chartModalState.data && chartModalState.data.length > 0 ? (
                chartModalState.data.map((item, idx) => (
                  <div key={idx} className="flex justify-between items-center p-3 rounded-xl bg-slate-800/80 dark:bg-slate-50 border border-slate-700/50 dark:border-slate-200">
                    <span className="font-bold text-slate-200 dark:text-slate-700 text-xs flex items-center gap-2">
                      <span className="w-3 h-3 rounded-full" style={{ backgroundColor: item.color }}></span>
                      {item.label}
                    </span>
                    <span className="px-3 py-1 rounded-lg bg-slate-900 dark:bg-white text-white dark:text-slate-800 font-black text-xs border border-slate-700 dark:border-slate-200">
                      {item.value}
                    </span>
                  </div>
                ))
                  ...
                  <div key={idx} className="bg-white/10 dark:bg-slate-50 p-4 rounded-2xl border border-red-500/20 shadow-lg relative overflow-hidden">
                    ...
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        ...
                        <span className="text-[10px] text-slate-400 dark:text-slate-500">القطاع: {comp.sector}</span>
                      </div>
                      <span className="bg-red-500/10 dark:bg-red-50 text-red-400 dark:text-red-600 text-[10px] font-bold px-2 py-1 rounded-full border border-red-500/20">
                        {comp.date || 'تاريخ غير محدد'}
                      </span>
                    </div>
                    <p className="text-xs text-slate-300 dark:text-slate-700 font-bold bg-slate-900 dark:bg-white p-3 rounded-xl border border-slate-700 dark:border-slate-200 mt-2">
                      {comp.details}
                    </p>
                    {comp.evidenceImage && (
                      <div className="mt-3 flex items-center gap-2 text-[10px] text-emerald-400 dark:text-emerald-600 bg-emerald-500/10 dark:bg-emerald-50 px-3 py-1.5 rounded-lg w-fit border border-emerald-500/20">
                        <Camera className="w-3.5 h-3.5" />
                        مرفق صورة إثبات المخالفة ({comp.evidenceImage})
                      </div>
                    )}
