...
export const EstablishmentsManager = () => {
  ...
  const getLiveStatus = (est) => {
    ...
    const hasApprovedFine = penaltyRequests && penaltyRequests.some(pr => (pr.establishmentId === est.id || pr.targetEstId === est.id || pr.estId === est.id || pr.estName === est.name) && pr.status === 'approved' && pr.type === 'fine' && pr.paymentStatus !== 'paid');
    const hasFinancialFine = fines && fines.some(f => (f.targetEstId === est.id || f.establishmentId === est.id || f.estId === est.id) && f.status !== 'paid');
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/40">
              {establishments
                ...
                .filter(e => {
                  ...
                  if (smartFilter === 'fined') return (penaltyRequests && penaltyRequests.some(pr => pr.establishmentId === e.id || pr.targetEstId === e.id || pr.estId === e.id || pr.estName === e.name)) || (fines && fines.some(f => f.targetEstId === e.id || f.establishmentId === e.id || f.estId === e.id));
                  return true;
                })
                .filter(e => 
