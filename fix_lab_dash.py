...
"""
    # Insert it right after useState for activeTab
    ...
    # We will replace the entire <nav className="space-y-2 mt-4"> block
    nav_start = content.find('<nav className="space-y-2 mt-4">')
    nav_end = content.find('</nav>', nav_start) + 6
    
    new_nav = """<nav className="space-y-2 mt-4">
    ...
    content = content.replace("activeTab === 'archive' ? 'الأرشيف المختبري' : 'المختبر المركزي'", "activeTab === 'archive' ? 'الأرشيف المختبري' : 'المختبر المركزي'")
    ...
    # Add the edit tab placeholder if edit is chosen
    # Since we don't have full edit logic, we can just show testingReqs with edit capability.
    # We will reuse the archive component or a simple message for now.
    edit_block = """
