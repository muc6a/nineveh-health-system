...
# 2. Replace the old storage block with the nested UI
# We need to find the old Settings block
...
# Extract everything before settings and after settings
before_settings = content.split(start_marker)[0]
after_settings = content.split(end_marker)[1] if end_marker in content else ""

new_content = before_settings + nested_storage_ui + end_marker + after_settings

# Make sure imports are present for ArrowRight, ArrowLeft, Image, Trash2, Archive
imports_to_add = ["ArrowRight", "ArrowLeft", "Image", "Trash2", "Archive"]
for imp in imports_to_add:
    if imp not in new_content:
        new_content = new_content.replace("} from 'lucide-react';", f", {imp} }} from 'lucide-react';")
