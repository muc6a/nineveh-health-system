...
# 2. Add storage sync
sync_target = """      } else if (e.key === 'dispatches' && e.newValue) {
        setDispatches(JSON.parse(e.newValue));
      }"""
sync_replace = """      } else if (e.key === 'dispatches' && e.newValue) {
        setDispatches(JSON.parse(e.newValue));
...
# 4. Add useRef and useEffect for localStorage
effect_target = """  useEffect(() => { if (isMountedFineTrans.current) syncToCloud('nineveh_fine_transactions', fineTransactions); else isMountedFineTrans.current = true; }, [fineTransactions]);"""
effect_replace = """  useEffect(() => { if (isMountedFineTrans.current) syncToCloud('nineveh_fine_transactions', fineTransactions); else isMountedFineTrans.current = true; }, [fineTransactions]);
